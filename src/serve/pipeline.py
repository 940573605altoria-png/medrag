...
class Pipeline:
    ...
    def __init__(self, config: AppConfig | None = None, *, qa_service: Any = None) -> None:
        self.config = config or load_config()
        # 注入真实 medical_qa（T047）；None 时 answer() 走骨架桩，向后兼容。
    ...
    def answer(self, question: str, top_k: int = 5) -> ReportResult:
        ...
        注入了真实 `qa_service`（T047 MedicalQA）则走真实链（hybrid 检索→精排→带引用回答）；
        否则退化为骨架桩（retrieve→stub_report），签名不变。
        """
        if self._qa_service is not None:
            return self._qa_service.answer(question, top_k)
...
def get_pipeline(config: AppConfig | None = None, *, qa_service: Any = None) -> Pipeline:
    """取默认管线单例；传 config 或 qa_service 则新建（消融/接真实 QA/测试用）。"""
    global _default_pipeline
    if config is not None or qa_service is not None:
        return Pipeline(config, qa_service=qa_service)
